...
const counts: Record<string, number> = Object.fromEntries(
  filters.map((item) => [item, item === "전체" ? talks.length : talks.filter((t) => t.kind === item).length])
);
  ...
  return (
    <section className="archive-block">
      <div className="archive-tools">
        <div className="filter-row" aria-label="발표 유형 필터">{filters.map((item) => <button className={filter === item ? "active" : ""} onClick={() => setFilter(item)} key={item}>{item}({counts[item]})</button>)}</div>
